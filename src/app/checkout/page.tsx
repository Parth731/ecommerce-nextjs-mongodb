...
import { addressAPIReponseType, cartReponseType } from "@/types/type";
...
export default function Checkout() {
  ...
  const publishableKey =
    "pk_test_51OmqYKSCF3hFuKRj126sKIPHZ6Cu5pfGkHFmaNcLWWFGJDe9qWxG5duGE8CvhunagQuyKaGkrvLmeXlxpgFs7Fkn00peoWn7dF";
    ...
    async function createFinalOrder() {
      ...
      if (
        ...
        const createFinalCheckoutFormData = {
          ...
          orderItems: cartItems.map((item: cartReponseType) => ({
            qty: 1,
            product: item.productID,
          })),
          paymentMethod: "Stripe",
          totalPrice: cartItems.reduce(
            (total: number, item: cartReponseType) =>
  ...
  async function handleCheckout() {
    ...
    const createLineItems = cartItems.map((item: cartReponseType) => ({
      price_data: {
        currency: "usd",
        product_data: {
        ...
        <div className="px-4 pt-8">
          ...
          <div className="mt-8 space-y-3 rounded-lg border bg-white px-2 py-4 sm:px-5">
            {cartItems && cartItems.length ? (
              cartItems.map((item: cartReponseType) => (
                <div
